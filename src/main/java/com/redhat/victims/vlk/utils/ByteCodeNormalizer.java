...
public class ByteCodeNormalizer implements ByteCodeNormalizerInterface {
    ...
    public ByteCodeNormalizer(ScannerConfigInterface config) {
        this.config = config;
    }
     ...
     * @return
     */
    private String constantValue(int index, ConstantPool cp) {
     ...
     * @throws IOException
     */
    private String formatBytecode(ByteSequence bytes, ConstantPool cp)
        throws IOException {
    ...
    public byte[] normalize(byte[] bytes, String fileName)
        ...
        return buf.toString().getBytes(config.getCharset());
    }
}
